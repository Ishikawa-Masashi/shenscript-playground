...
export default (
  Component: ({
    ...
    children?: string;
    errorColor?: string;
    renderError?: (error: TypeError) => JSX.Element;
    displayMode?: boolean;
  }) => JSX.Element,
  ...
  const expectToContainCustomError = (
    container: HTMLElement,
    customError: string
  ) => expect(container.querySelector('.error')).toContain(customError);

  // const expectToContain = (content: string) =>
  //   expect(getReactKatexElement()).toContainHTML(content);

  const expectToContain = (content: string) =>
    expect(getReactKatexElement()).toContain(content);

  const expectToContainFormula = (
    formula: string,
    { displayMode }: { displayMode?: boolean } = {}
  ) => expectToContain(KaTeX.renderToString(formula, { displayMode }));
  ...
  describe('error handling', () => {
    it('updates when passing from invalid to valid formula', () => {
      const { rerender } = render(
        <Component math={invalidCommandFormula} renderError={renderError} />
      );
    ...
    it('updates when passing from valid to invalid formula', () => {
      const { rerender, container } = render(
        <Component math={integralFormula} renderError={renderError} />
      );
    ...
    describe('when using default error handler', () => {
      ...
      describe('when passing custom error color', () => {
        it('renders the formula with the wrong part highlighted in custom color', () => {
          render(
            <Component errorColor={'blue'} math={invalidCommandFormula} />
          );
      ...
      describe('when error is caused while parsing math expression', () => {
        it('renders error message', () => {
          ...
          expectToContain(
            "KaTeX parse error: Expected '}', got 'EOF' at end of input: \\sum_{"
          );
        });
      });
    });
